...
int main ()
{
  ...
  //Create pid_throttle for throttle command
  ...
  //Create pid_steer for steering command
  PID pid_steer = PID();
  
  // initialize pid_steer
  double steer_kpi = 0.1;
  double steer_kii = 0.0002;
  double steer_kdi = 0.15;
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          //Compute error_steer from desired trajectory
          ...
          **/
          ...
          //The last point of v_points vector contains the velocity calculated by the path planner
          // The actual velocity is provided by the velocity variable
          // To calculate the error we need to substract the expected velocity from the actual velocity
